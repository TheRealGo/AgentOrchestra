...
class StopHookAndTmuxTests(NoWakeSleepMixin, unittest.TestCase):
    def test_wake_command_pastes_fixed_runtime_payload_with_named_buffer_and_submit_key(self) -> None:
        fake = FakeTmux()

        result = send_wake("%7", runner=fake, sleeper=_no_sleep)

        wake_buffer = self.wakeBuffer(fake)
        self.assertTrue(result.accepted)
        # Fire-and-forget: paste then three submit-key presses, no capture/confirm.
        self.assertEqual(
            fake.calls,
            [
    ...
    def test_wake_is_fire_and_forget_without_poll_confirm(self) -> None:
        # The Codex wake polled the pane and could report a stale/unaccepted
        # paste. The Claude wake cannot confirm (it would deadlock on its own
        # Stop Hook), so it issues a fixed number of submit presses and always
        # reports accepted, never capturing the pane.
        fake = FakeTmux()

        result = send_wake("%7", runner=fake, sleeper=_no_sleep)

        send_keys = [call for call in fake.calls if call[0][:2] == ["tmux", "send-keys"]]
        self.assertTrue(result.accepted)
        self.assertEqual(result.attempts, WAKE_SUBMIT_PRESSES)
        self.assertEqual(len(send_keys), WAKE_SUBMIT_PRESSES)
    ...
    def test_wake_command_allows_configured_submit_key(self) -> None:
        ...
        send_wake("%7", submit_key="C-m", runner=fake, sleeper=_no_sleep)
    ...
    def test_wake_command_defaults_blank_submit_key(self) -> None:
        ...
        send_wake("%7", submit_key="  ", runner=fake, sleeper=_no_sleep)

...
class SkillBoundaryContractTests(unittest.TestCase):
    def test_launch_skill_uses_protocol_layers_for_professional_agents(self) -> None:
        launch = (CLAUDE / "skills" / "agent-orchestra-launch" / "SKILL.md").read_text(encoding="utf-8")
    ...
    def test_tmux_skills_are_split_by_common_and_main_operations(self) -> None:
        common = (CLAUDE / "skills" / "agent-orchestra-tmux-common" / "SKILL.md").read_text(encoding="utf-8")
        main = (CLAUDE / "skills" / "agent-orchestra-tmux-main" / "SKILL.md").read_text(encoding="utf-8")
        ...
        self.assertIn("slow Claude Code TUI startup", common)
        self.assertIn("peer still finishing its current turn", common)
        self.assertIn("not supervision", common)
        self.assertIn("returns non-zero if the target Claude Code TUI does not accept", common_normalized)
    ...
    def test_task_file_skill_documents_empty_done_vs_active_progress(self) -> None:
        task = (CLAUDE / "skills" / "agent-orchestra-task-file" / "SKILL.md").read_text(encoding="utf-8")
    ...
    def test_main_template_documents_requested_mainagent_self_exit(self) -> None:
        template = (CLAUDE / "agent_orchestra_minimal" / "agent_templates" / "main.CLAUDE.md").read_text(
            encoding="utf-8"
        )

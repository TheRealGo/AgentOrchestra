...
class LaunchMaterialHelperIOTests(unittest.TestCase):
    def test_installed_helper_prepares_professional_launch_material_from_main_env(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            helper = main.claude_config_dir / "agent_orchestra_minimal" / "prepare_agent_launch.py"
            ...
            self.assertEqual(pro_env["PYTHONPATH"], str(pro_dir / "claude_home"))
            self.assertNotIn("CODEX_HOME", pro_env)
            self.assertEqual(pro_env["CLAUDE_CONFIG_DIR"], str(pro_dir / "claude_home"))
    ...
    def test_installed_helper_resolves_protocol_layer_from_main_env(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            startup = (pro_dir / "workspace" / "CLAUDE.md").read_text(encoding="utf-8")
            pro_env = json.loads((pro_dir / "env.json").read_text(encoding="utf-8"))
    ...
    def test_installed_helper_prefers_protocol_layer_over_target_layer(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            startup = (main.run_dir / "agents" / "pro-backend" / "workspace" / "CLAUDE.md").read_text(
                encoding="utf-8"
            )

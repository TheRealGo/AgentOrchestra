    ...
    def test_readmes_describe_team_task_state_and_delivery_contracts(self) -> None:
        required_by_readme = {
            "README.md": (
                ...
                "every `[Acceptance]` item is satisfied, out-of-scope, or deferred with evidence",
                "every `[Gates]` item is passed or explicitly non-applicable",
                "Acceptance or gate items marked `blocked` or `needs_user` keep the run at `[status] progress`",
            ...
            "README.ja.md": (
                ...
                "`[Acceptance]` の各項目が satisfied / out-of-scope / deferred",
                "`[Gates]` の各項目が passed または明示的な non-applicable",
                "blocked/needs_user は外部アクション待ちの未完了状態",

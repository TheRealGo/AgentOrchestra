...
class SpecContractTests(unittest.TestCase):
    ...
    def test_spec_requires_team_sufficiency_not_blanket_launching(self) -> None:
        ...
        required_phrases = [
            ...
            "tmux Main Skill self-exit procedure as its final tool action", "uses a packaged helper", "writes JSON evidence", "clears visible `/exit` leftovers",
            ...
            "resolve Agent state `tmux_target` instead of using a bare active-pane lookup",
    ...
    def test_completion_criteria_include_current_runtime_gates(self) -> None:
        ...
        required_phrases = [
            ...
            "user-requested MainAgent self-exit uses the tmux Main Skill self-exit procedure", "writes JSON evidence", "clears visible `/exit` leftovers",
            ...
            "resolves the MainAgent pane from `AGENT_ORCHESTRA_TMUX_PANE` or Agent state `tmux_target`",
            "never from an unqualified active-pane lookup",

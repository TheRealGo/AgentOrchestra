...
not use delayed or background self-exit shell jobs; under Claude Code TUI
turns they can fail before sending the key.

Use a detached Python self-exit as the final tool action so the submit happens
after the active Claude Code turn has yielded back to the TUI. Send the submit key

...
def _last_prompt_marker_index(lines: list[str]) -> int:
    for index in range(len(lines) - 1, -1, -1):
        if lines[index].lstrip().startswith(PROMPT_MARKERS):
            return index
    return -1


# Claude Code TUI composer/user-message lines start with U+276F "❯". A bare ">"
# is kept as a defensive fallback.
PROMPT_MARKERS = ("❯", ">")
# Empirically captured Claude Code v2.1.x working/output line starts: assistant
# and tool output begin with U+23FA "⏺ "; the working/done status line begins
# with a rotating spinner glyph (for example "✻ Frolicking…" or "✻ Baked for
# 10s").
START_LINE_MARKERS = ("⏺ ", "✻ ", "✽ ", "✶ ", "✢ ", "✳ ", "∗ ", "✺ ", "● ")
START_SUBSTRINGS = ("esc to interrupt",)
# Messages submitted while Claude Code is busy are held back; these hints mark a
# queued (not yet delivered) message in the composer area.
QUEUE_SUBSTRINGS = ("message queued", "messages queued", "tab to queue")


def _looks_started(capture: str) -> bool:
    lines = capture.splitlines()
    if any(_line_has_start_marker(line, START_LINE_MARKERS) for line in lines):
        return True
    return any(substring in line for line in lines for substring in START_SUBSTRINGS)
...
def _line_has_queue_marker(line: str) -> bool:
    return any(marker in line for marker in QUEUE_SUBSTRINGS)

...
from agent_orchestra_minimal.launch_args import forwarded_auth_env
...
def main(argv: list[str] | None = None) -> int:
    ...
    doctor.add_argument(
        "--tui-transport",
        action="store_true",
        help="verify Claude Code TUI tmux delivery with a disposable /exit probe",
    )
...
def start_main(args: argparse.Namespace) -> int:
    ...
    _warn_if_no_credential(material)
    if args.dry_run:
    ...
    os.execvpe("claude", material.command["argv"], env)
    return 0


def _warn_if_no_credential(material: LaunchMaterial) -> None:
    """Warn when no credential will reach the isolated Agent.

    The isolated ``CLAUDE_CONFIG_DIR`` does not read the macOS Keychain (verified
    empirically: a launch into a fresh config reports "Not logged in" even with a
    real ``HOME`` and a seeded ``oauthAccount``), so a normal ``claude`` login
    does not carry over. A credential reaches the Agent only via a forwarded
    token env var or a copied ``.credentials.json``.
    """

    if forwarded_auth_env():
        return
    if (material.claude_config_dir / ".credentials.json").is_file():
        return
    print(
        'agent-orchestra: warning: no Claude Code credential is available to the '
        'isolated runtime, so the Agent will start "Not logged in" and cannot '
        "work until you supply one.\n"
        "  The isolated CLAUDE_CONFIG_DIR does not read the macOS Keychain, so a "
        "normal `claude` login does not carry over.\n"
        "  Fix (one-time): run `claude setup-token`, then `export "
        "CLAUDE_CODE_OAUTH_TOKEN=...` (add it to ~/.zshrc to persist) and re-run "
        "claude-o. One exported credential reaches MainAgent and every "
        "ProfessionalAgent via the forwarded env.sh.",
        file=sys.stderr,
    )
...
def parse_start_args(argv: list[str]) -> argparse.Namespace:
    ...
    if "--" in argv:
        parser.error("initial task arguments are not supported; start claude-o and enter the task in the TUI")

...
scoped task in the shared task file as awaiting review. Move your scoped task to
`Done` only when the accepted disposition is known; do not use this task update
to decide whole-run completion. If peer or MainAgent consultation would improve
the specialist result, use
...
not run or request `pytest` unless the user explicitly asked for it or you first
...
when they improve depth, critique, implementation confidence, or evidence
review. Before `ready_for_review` on
